  ...
  return (
    <div ref={pageRef} id="community" className="scroll-mt-20 bg-[#08090D]">
      ...
      <div className="mx-auto max-w-7xl px-6 py-28 lg:px-16">
        <div className="flex flex-col gap-16">
          <div className="comm-heading flex flex-col gap-3">
            ...
            <h2 className="font-heading text-4xl font-bold text-white md:text-5xl">Four steps to being part of it.</h2>
          </div>
          ...
          <div className="flex flex-col divide-y divide-white/8">
            {steps.map(({ num, title, desc }) => (
              <div key={num} className="comm-step grid grid-cols-[auto_1fr] gap-8 py-12 md:grid-cols-[80px_1fr_1.2fr] md:gap-16 md:items-center">
                <span className="font-heading text-sm font-bold text-white/20">{num}</span>
                <h3 className="font-heading text-2xl font-bold text-white md:text-3xl">{title}</h3>
                <p className="col-span-2 text-sm leading-relaxed text-white/45 md:col-span-1">{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="relative overflow-hidden bg-[#111318] px-6 py-28 lg:px-16">
        <div className="pointer-events-none absolute -top-20 -right-20 h-96 w-96 text-[#D4AF37] opacity-[0.04]">
              ...
              return (
                <div
                  ...
                  className={`comm-tier relative flex flex-col gap-6 overflow-hidden rounded-3xl p-8 ${
                    highlight ? "bg-[#D4AF37] text-[#08090D]" : "bg-white/4 border border-white/6 text-white"
                  }`}
                >
                  <div className={`pointer-events-none absolute -bottom-10 -right-10 h-48 w-48 ${highlight ? "text-[#08090D] opacity-[0.08]" : "text-[#D4AF37] opacity-[0.06]"}`}>
                    <Ghost className="h-full w-full" />
                  </div>
                  <div className="flex items-start justify-between">
                    <div className="flex flex-col gap-1">
                      <span className={`text-xs font-semibold uppercase tracking-widest ${highlight ? "text-[#08090D]/50" : "text-white/30"}`}>Tier</span>
                      <span className="font-heading text-2xl font-bold">{name}</span>
                    </div>
                    <div className={`rounded-2xl px-3 py-1.5 text-xs font-bold ${highlight ? "bg-[#08090D]/15" : "bg-white/8"}`}>
                      {amount} RARE
                    </div>
                  </div>
                  <ul className="flex flex-col gap-2.5">
                    {perks.map((perk) => (
                      <li key={perk} className={`flex items-center gap-2.5 text-sm ${highlight ? "text-[#08090D]/75" : "text-white/50"}`}>
                        <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${highlight ? "bg-[#08090D]" : "bg-[#D4AF37]/50"}`} />
                        {perk}
                      </li>
                    ))}
                  </ul>
      ...
      <div className="mx-auto max-w-7xl px-6 py-28 lg:px-16">
        <div className="flex flex-col gap-16">
          <div className="comm-heading flex flex-col gap-3">
            <span className="text-xs font-semibold uppercase tracking-widest text-[#D4AF37]">Our Principles</span>
            <h2 className="font-heading text-4xl font-bold text-white md:text-5xl">What this community stands for.</h2>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {principles.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="comm-principle relative flex flex-col gap-4 overflow-hidden rounded-3xl bg-[#111318] border border-white/6 p-8">
      ...
      <div className="mx-auto max-w-7xl px-6 py-28 lg:px-16">
        <div className="flex flex-col gap-12">
          <div className="comm-heading flex flex-col gap-3">
            ...
            <h2 className="font-heading text-4xl font-bold text-white md:text-5xl">Join the conversation.</h2>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {socials.map(({ icon: Icon, label, handle, href }) => (
              <a
                ...
                className="comm-social group flex flex-col gap-5 rounded-3xl bg-[#111318] border border-white/6 p-8 transition-all hover:border-[#D4AF37]/30 hover:-translate-y-0.5"
              >
                <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/6 transition-colors group-hover:bg-[#D4AF37]/10">
                  <Icon size={20} className="text-white/40 transition-colors group-hover:text-[#D4AF37]" strokeWidth={1.75} />
                </div>
                <div className="flex flex-col gap-1">
                  <span className="font-heading text-base font-bold text-white">{label}</span>
                  <span className="text-sm text-white/35">{handle}</span>
                </div>
                <ArrowRight size={15} className="text-white/20 transition-all group-hover:text-[#D4AF37] group-hover:translate-x-1" />
              </a>
            ))}
          </div>
        </div>
